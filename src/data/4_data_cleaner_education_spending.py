...
relative_path = 'raw/educationspending_billions.csv'
...
for line in file_array:
    for datum in all_data:
        if (line[0] == datum[0]):
            datum.append(float(line[1]) * 1000)
...
relative_path = 'data_plotter4.csv'
file_path = os.path.join(dir_name, relative_path)
new_file = open(file_path, 'w')
old_file = open(os.path.join(dir_name, 'data_plotter3.csv'))
